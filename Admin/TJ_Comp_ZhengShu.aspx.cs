...
using TJ.BLL;
using commonlib;
public partial class Admin_TJ_Comp_ZhengShu : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_Comp_ZhengShuAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+"&ID={0}',600,680,'资质证书')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
       ...
       AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(id) from TJ_Comp_ZhengShu where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_Comp_ZhengShu", _filtertemp, "id", "id", pageSize);
       GridView1.DataBind();
    }
