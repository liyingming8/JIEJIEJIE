...
public partial class Admin_TJ_Activity_Strategy : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_Activity_StrategyAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+"&ID={0}',580,450,'奖励策略')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(id) from TJ_Activity_Strategy where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_Activity_Strategy", _filtertemp, "id", "id", pageSize);
       GridView1.DataBind();
    }
