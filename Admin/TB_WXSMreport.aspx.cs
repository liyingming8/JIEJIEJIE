...
public partial class TB_WXSMreport : AuthorPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            txtStartDate.Value = DateTime.Now.Year+"-01";
            txtEndDate.Value = Convert.ToDateTime(DateTime.Now.Year + "-" + DateTime.Now.Month).ToString("yyyy-MM");
            string endtime = Convert.ToDateTime(txtEndDate.Value).AddMonths(1).ToString("yyyy-MM-dd");
            string startime = Convert.ToDateTime(txtStartDate.Value).ToString("yyyy-MM-dd"); 
            DataTable sml = db.GetsmcsForAllMonth(startime, endtime, GetCookieCompID(),DAConfig.Showmode);
            if (sml.Rows.Count > 0)
            {
                Session.Add("sml", sml);
                string cs = "|";
