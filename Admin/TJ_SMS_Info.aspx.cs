...
using TJ.BLL;
using commonlib;
public partial class Admin_TJ_SMS_Info : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_SMS_InfoAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+ "&ID={0}',720,500,'短信提醒设置')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        AspNetPager1.RecordCount =
            int.Parse(tab.ExecuteQuery("select count(id) from TJ_SMS_Info where " + _filtertemp, null).Rows[0][0]
                .ToString());
        AspNetPager1.CurrentPageIndex = pageIndex;
        GridView1.DataSource =
            tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_SMS_Info", _filtertemp, "id", "id", pageSize);
        GridView1.DataBind();
    }
