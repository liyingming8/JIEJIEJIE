...
using TJ.BLL;
using commonlib;
public partial class Admin_TJ_SWM_Comp_ModulesConfigForAdmin : AuthorPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        { 
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["pcompid"]))
            {
                ...
                inputaddmodule.Attributes.Add("onclick", string.Format("openWinCenter('SystemAuthorManageForSWM.aspx?compid={0}',600,500,'功能授权')", Sc.EncryptQueryString(hd_compid.Value)));
            }
            if (!string.IsNullOrEmpty(Request.QueryString["pcompnm"]))
            {
                inputcompid.Value = Request.QueryString["pcompnm"];
            }
            inputcompid.Attributes.Add("onclick", ReturnCompnaySelectScript("指定三维码客户", "0", "UseSWM=1")); 
            _currentindex = 1;
    ...
    {
        if (!string.IsNullOrEmpty(hd_compid.Value) && !hd_compid.Value.Equals("0"))
        {
            _filtertemp = "compid=" + hd_compid.Value;
            ...
            AspNetPager1.RecordCount =
                int.Parse(
                    tab.ExecuteQuery("select count(id) from TJ_SWM_Comp_ModulesConfig where " + _filtertemp, null).Rows[
                        0][0].ToString());
            GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_SWM_Comp_ModulesConfig", _filtertemp,
                "id", "id", pageSize);
            GridView1.DataBind();
        }
    }
