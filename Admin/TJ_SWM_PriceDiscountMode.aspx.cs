...
using TJ.BLL;
using commonlib;
public partial class Admin_TJ_SWM_PriceDiscountMode : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_SWM_PriceDiscountModeAddEdit.aspx?cmd=" + Sc.EncryptQueryString("edit") + "&ID={0}',600,450,'三维码折扣模式')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
       ...
       AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(id) from TJ_SWM_PriceDiscountMode where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_SWM_PriceDiscountMode", _filtertemp, "id", "id", pageSize);
       GridView1.DataBind();
    }
