...
using TJ.Model;
using commonlib;
public partial class Admin_TJ_CompTheme_color : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_CompTheme_colorAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+"&ID={0}',600,450,'TJ_CompTheme_color')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
       ...
       AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(id) from TJ_CompTheme_color where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TJ_CompTheme_color", _filtertemp, "id", "id", pageSize);
       GridView1.DataBind();
    }
