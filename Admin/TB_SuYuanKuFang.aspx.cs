...
public partial class Admin_TB_SuYuanKuFang : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TB_SuYuanKuFangAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+"&ID={0}',500,320,'库房信息')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
       ...
       AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(ID) from TB_SuYuanKuFang where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "TB_SuYuanKuFang", _filtertemp, "ID", "ID", pageSize);
       GridView1.DataBind();
    }
