...
public partial class Admin_TJ_Depart_UserSelectByRID : AuthorPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        { 
            if(!string.IsNullOrEmpty(Request.QueryString["RID"])&&!string.IsNullOrEmpty(Request.QueryString["fr"]))
            {
                rid.Value = Sc.DecryptQueryString(Request.QueryString["RID"]);
    ...
    { 
        if (IsSuperAdmin())
        {
            _filtertemp = "departid=" + GetCookieTJDepartID() + " and RID=" + rid.Value+ " CompID<>0";
            if (inputSearchKeyword.Value.Length > 0)
            {
        ...
        else
        {
            string tempagentid = comwl.GetAgentIDStringByCompID(GetCookieCompID());
            string tempchildcompid = comfun.ReturnChildCompIDString(GetCookieCompID(), true);
            _filtertemp = "departid=" + GetCookieTJDepartID() + " and RID="+rid.Value+" and CompID<>0 and (CompID in (" + tempagentid + ") or CompID in (" + tempchildcompid + ") or CompID=" + GetCookieCompID() + ")"; 
            if (inputSearchKeyword.Value.Length > 0)
            {
                _filtertemp += " and " + DDLField.SelectedValue + " like '%" + inputSearchKeyword.Value + "%'";
            } 
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (dataKey != null)
            {
                e.Row.Attributes.Add("onclick", XiangXiLinkStringForReturn(dataKey["UserID"].ToString(), dataKey["LoginName"].ToString(), dataKey["RID"].ToString()));
            }   
        }
    }

    public string XiangXiLinkStringForReturn(string userid,string username,string userrid)
    {
        if (userid.Length > 0)
        {
            return
                string.Format("javascript:closemyWindowReloadNewhref('" + fr.Value + "?UID=" + uid.Value + "&suid=" +
                              Sc.EncryptQueryString(userid) + "&srid=" + Sc.EncryptQueryString(userrid) + "&sunm="+Sc.EncryptQueryString(username)+"')");
        }
        return "";
    }
