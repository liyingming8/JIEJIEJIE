...
public partial class TJ_UserJFlab : AuthorPage
{
    ...
    private void fillgridview(string userid, string compid)
    {
        if (userid != "")
        {
            GridView1.DataSource = db.getJFlab(ReturnFilterString(userid));
            GridView1.DataBind();
        }

        else
        {
            GridView1.DataSource = db.getJFlab(ReturnFilterStringNuser());
            GridView1.DataBind();
        }
    }
    ...
    private string ReturnFilterStringNuser()
    {
        ...
        {
            tempstring += " and " + "JFTime>='" + DateBegin.Text + "' and JFTime<'" +
        ...
        if (inputSearchKeyword.Value.Length > 0)
        {
            ...
            tempstring += "and UserID=" + uid + "";
        }

        tempstring += "order by JFTime";
        return tempstring;
    }

    private string ReturnFilterString(string userid)
    {
        tempstring = "CompID=" + GetCookieCompID() + " and UserID=" + userid + "";
        if (DateBegin.Text != "" && DateEnd.Text != "")
        {
            tempstring += " and " + "JFTime>='" + DateBegin.Text + "' and JFTime<'" +
                          Convert.ToDateTime(DateEnd.Text).AddDays(1) + "'";
