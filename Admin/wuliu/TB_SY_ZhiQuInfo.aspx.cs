...
public partial class Admin_TB_SY_ZhiQuInfo : AuthorPage
{
    private readonly BTB_SY_ZhiQuInfo bll = new BTB_SY_ZhiQuInfo();
    private MTB_SY_ZhiQuInfo mod = new MTB_SY_ZhiQuInfo();
    public BTB_SY_YuanLiangInfo byl = new BTB_SY_YuanLiangInfo();
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_SY_ZhiQuInfoAddEdit.aspx?cmd=edit&ID={0}',600,400,'制曲管理')", ID);
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_SY_ZhiQuInfo where " + Filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_SY_ZhiQuInfo", Filtertemp, "ZhiQuID", "ZhiQuID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["ZhiQuID"].ToString()));
        //fillgridview();
        _currentindex = 1;
        AspNetPager1.CurrentPageIndex = _currentindex;
        DisplayData(1, AspNetPager1.PageSize);
