...
public partial class Admin_TB_WorkLineInfo : AuthorPage
{
    private readonly BTB_WorkLineInfo bll = new BTB_WorkLineInfo();
    private MTB_WorkLineInfo mod = new MTB_WorkLineInfo();
    public BTB_WorkShopInfo bworkshop = new BTB_WorkShopInfo();
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_WorkLineInfoAddEdit.aspx?cmd=edit&ID={0}',450,300,'厂房信息')", ID);
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_WorkLineInfo where " + filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_WorkLineInfo", filtertemp, "WLID", "WLID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["WLID"].ToString()));
       // fillgridview();
        _currentindex = 1;
        AspNetPager1.CurrentPageIndex = _currentindex;
        DisplayData(1, AspNetPager1.PageSize);
    ...
    {
        mod = bll.GetList(int.Parse(GridView1.DataKeys[e.RowIndex]["WLID"].ToString()));
        mod.WorkLineName = ((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtWorkLineName")).Text.Trim();
