...
public partial class Admin_TB_ProductXiangXing : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_ProductXiangXingAddEdit.aspx?cmd=edit&ID={0}',450,300,'香型编辑')", ID);
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_ProductXiangXing where " + Filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_ProductXiangXing", Filtertemp, "ID", "ID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        mod.XiangXing = ((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtXiangXing")).Text.Trim();
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //{
        //    if (!bproduct.CheckIsExistByFilterString("ProductXiangXing="+GridView1.DataKeys[e.Row.RowIndex][0].ToString()))
        //    {
