...
public partial class Admin_TB_ProductJingHanLiang : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_ProductJingHanLiangAddEdit.aspx?cmd=edit&ID={0}',450,300,'净含量编辑')", ID);
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_ProductJingHanLiang where " + Filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_ProductJingHanLiang", Filtertemp, "ID", "ID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //{
        //    if (!bproduct.CheckIsExistByFilterString("ProductJingHanLiang=" + GridView1.DataKeys[e.Row.RowIndex][0].ToString()))
        //    {
