...
public partial class Admin_wuliu_Fahuo_FaHuoDetialjg : AuthorPage
{
    ...
    protected void Button_Search_Click(object sender, EventArgs e)
    {
        ...
        if (db.SelectFhTable(HF_AgentID.Value).Rows[0][0].ToString() == "1")
        {
            string sqlstring =
                ...
                HF_AgentID.Value +
                "] fh,TB_Products_Infor pr,TB_StoreHouse st where   fh.FHTypeID=3 and fh.ProID=pr.Infor_ID and fh.STID=st.STID and fh.FHDate  between '" +
                TextBox_RukuDateBegin.Text + "' and '" + Convert.ToDateTime(TextBox_RukuDateEnd.Text) + "'";
        ...
        if (Session["starTime"] != null)
        {
            ...
            if (db.SelectFhTable(HF_AgentID.Value).Rows[0][0].ToString() == "1")
            {
                string sqlstring =
                    ...
                    HF_AgentID.Value +
                    "] fh,TB_Products_Infor pr,TB_StoreHouse st where   fh.FHTypeID=3 and fh.ProID=pr.Infor_ID and fh.STID=st.STID and fh.FHDate  between '" +
                    star + "' and '" + Convert.ToDateTime(end).AddDays(1) + "'";
