...
public partial class Admin_wuliu_Fahuo_FahuoNewno_XB : AuthorPage
{
        ...
        if (RadioButtonList_Mode.SelectedValue.Equals("1") || RadioButtonList_Mode.SelectedValue.Equals("3"))
        {
            ...
            if (Session["dttempline"] != null)
            {
                ...
                if (!repeatstring.Equals(string.Empty))
                {
                    ...
                    {
                        Label2.Text = "通过检查,上传数据中有" + Convert.ToString(dttempline.Rows.Count - repnum) + "组有效数据," +
                                      repnum + "组重复数据";
                ...
                {
                    Label2.Text = "通过检查,上传数据中有" + Convert.ToString(dttempline.Rows.Count) + "条数据";
                ...
                else
                {
                    ...
                    if (string.IsNullOrEmpty(TextBox_ChuKuDanHao.Value.Trim()))
                    {
                        fahuoinfo[0] = DateTime.Now.ToString("yyyyMMddhhmmss");
                            //TextBox_ChuKuDanHao.Value.Trim();//发货批次
                    }
                    else
                    {
                    ...
                    if (datarows.Length > 0)
                    {
                        ...
                        foreach (DataRow dr in datarows)
                        {
                            ...
                            if (RadioButtonList_Mode.SelectedValue.Equals("3"))
                            {
                                if (datarows.Length > 0)
                                {
                                    foreach (DataRow drth in datarows)
                                    {
                                        DataTable dtth = db.GetFhPiciandFhKeyNOXB(drth[3].ToString().Trim(),
                                            GetCookieCompID());
                                        if (CheckIsPrF(drth[3].ToString().Trim()) && dtth.Rows.Count > 0)
                                        {
                                            string pici = dtth.Rows[0]["FHPici"].ToString();
                                            string key = dtth.Rows[0]["FHKey"].ToString();
                                            sqlcmd3.CommandText = GetTuiHuoString(drth[3].ToString().Trim(), pici, key);
                            ...
                            {
                                if (CheckIsPrF(dr[3].ToString().Trim()))
                                {
                                    isok = true;
                                }
                                else
                                {
                                    sqlcmd2.CommandText = GetFaHuoSqlString(dr[3].ToString().Trim(), ProductInfoID,
                                        GetCookieCompID(), AgentID, GetCookieUID(), FaHuoPiCi, StoreHouseID,
                                        fahuoinfo[3], AgentID, EnterFaHuoDate);
                        ...
                        if (oknum > 0)
                        {
                            ...
                            foreach (string line in FaHuoRecordArray)
                            {
                                if (!line.Trim().Equals(""))
                                {
                                    ...
                                    if (EnterFaHuoDate)
                                    {
                                        RecordFaHuoInfo("TB_FaHuoInfo_NOXB_" + GetCookieCompID(), temparray[0].Trim(),
                                            TextBox_FaHuoTime.Text, temparray[2].Trim(), sqlcon1, temparray[3].Trim(),
                        ...
                        if (RadioButtonList_Mode.SelectedValue.Equals("3"))
                        {
                            ...
                            {
                                ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info", "alert('退货完成!')",
                                    true);
                                Label2.Text = "";
                                Session.Remove("dttempline");
                                Session.Remove("strList");
                            ...
                            {
                                Label2.Text = "本次退货过程中发生异常!实际退货数量为:" + thnum + "件,异常:" + (datarows.Length - thnum) +
                                              "件,请仔细排查!";
                                ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info",
                                    "alert('本次退货过程中发生异常!实际退货数量为:" + thnum + "件,上传文件中有如下号码存在异常:" + ExceptionString.Trim() +
                                    ",请仔细排查!');", true);
                        ...
                        else
                        {
                            if (ExceptionString.Trim().Equals(""))
                            {
                                ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info", "alert('发货完毕!')",
                                    true);
                                Label2.Text = "";
                                Session.Remove("dttempline");
                                Session.Remove("strList");
                            ...
                            else
                            {
                                Label2.Text = "本次发货过程中发生异常!实际发货数量为:" + boxnum + "件,异常:" + (datarows.Length - boxnum) +
                                              "件,请仔细排查!";
                                ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info",
                                    "alert('本次发货过程中发生异常!实际发货数量为:" + boxnum + "件,上传文件中有如下号码存在异常:" +
                                    ExceptionString.Trim() + ",请仔细排查!');", true);
                                Label_Exceptions.Visible = true;
                                TextBox_ExceptionLines.Text = ExceptionString.Trim();
                                TextBox_ExceptionLines.Visible = true;
                                Session.Remove("dttempline");
    ...
    { 
        ...
        sqlforinsertrukuinfo = "insert into TB_FaHuoInfo_" + GetCookieCompID() +
                               ...
                               "','" + FHKey + "'," + ProID + "," + XiangNumber + "," + GetCookieCompID() + "," + STID +
                               ")";
    ...
    /// <param name="FHKey"></param>
    /// <returns></returns>
    private string GetTuiHuoString(string boxstring, string FaHuoPiCi, string FHKey)
    {
        string stringtemp = string.Empty;


        stringtemp = "update  TB_FaHuoInfo_" + GetCookieCompID() + " set XiangNumber=XiangNumber-1  where FHPiCi='" +
                     FaHuoPiCi + "' and FHKey='" + FHKey + "' ; delete  from  TB_FaHuoInfo_NOXB_" + GetCookieCompID() +
                     " where BoxLabel01='" + boxstring + "' and CompID=" + GetCookieCompID() +
                     "and FHTypeID=3 ; delete  TB_FaHuoInfo_" + GetCookieCompID() + " where XiangNumber=0 ";
    ...
    /// <returns></returns>
    private string GetFaHuoSqlString(string boxstring, string ProID, string compid, string MuDiDi, string fhuser,
        string FaHuoPiCi, string StoreHouseID, string FHKey, string AgengtID, bool InputDateTime)
    {
        string stringtemp = string.Empty;
        //if (RadioButtonList_Mode.SelectedValue.Equals("1"))
        ...
        if (InputDateTime)
        {
            stringtemp = " insert into  TB_FaHuoInfo_NOXB_" + compid +
                         "(BoxLabel01,ProID,AgentID,STID,FHDate,FHTypeID,FHUserID,FHPici,FHKey,CompID) values('" +
                         boxstring + "'," + ProID + "," + AgengtID + "," + StoreHouseID + ",'" + TextBox_FaHuoTime.Text +
                         "',3," + GetCookieUID() + ",'" + FaHuoPiCi + "','" + FHKey + "'," + GetCookieCompID() + ")";
        }
        else
        {
            stringtemp = " insert into  TB_FaHuoInfo_NOXB_" + compid +
                         "(BoxLabel01,ProID,AgentID,STID,FHDate,FHTypeID,FHUserID,FHPici,FHKey,CompID) values('" +
                         boxstring + "'," + ProID + "," + AgengtID + "," + StoreHouseID + ",'" + DateTime.Now + "',3," +
                         GetCookieUID() + ",'" + FaHuoPiCi + "','" + FHKey + "'," + GetCookieCompID() + ")";
        }
        //}
        //else
        //{
    ...
    /// <returns></returns>
    public bool CheckIsPrF(string BoxLabelString)
    {
        string sqlstring = "select count(BoxLabel01) as num from  TB_FaHuoInfo_NOXB_" + GetCookieCompID() +
                           " where BoxLabel01='" + BoxLabelString + "'and FHTypeID=3";
    ...
    public bool CheckWfhISok(string tablename, string BoxLabelString)
    {
        string sqlstring = "select count(BoxLabel01) as num from " + tablename + "_FH where BoxLabel01='" +
                           BoxLabelString + "'and left(FHType,1)=3 and CompID=" + GetCookieCompID() + "";
        sqlcmdforcheck.CommandText = sqlstring;
        sqlcmdforcheck.Connection = sqlconcom;
        opensqlconn(sqlconcom);
        SqlDataReader sdr = sqlcmdforcheck.ExecuteReader();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        Session.Remove("strList");
        ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info",
            "alert('清除成功！');window.location.href='FahuoNewno_XB.aspx'", true);
    }
}