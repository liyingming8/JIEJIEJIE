...
public partial class Admin_wuliu_Fahuo_FahuoNew01 : AuthorPage
{
    ...
    private readonly commwl comm = new commwl();
    ...
    {
        Zwmark.Attributes.CssStyle.Value = "display:block";
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (datarows.Length > 0)
                    {
                        ...
                        tablenames =
                            comm.gettableinfo(HFMaxMinValue.Value.Split(',')[0].Trim(),
                                HFMaxMinValue.Value.Split(',')[1].Trim(), sqlcon1).Split(',');
                        ...
                        if (tablenames.Length > 1)
                        {
                            foreach (DataRow dr in datarows)
                            {
                                ...
                                if (!CheckBox_SelectProductInfo.Checked)
                                {
                                    if (!ProductInfoCode.Equals(dr[2].ToString().Trim()) ||
                                        ProductInfoID.Trim().Equals(""))
                                    {
                                ...
                                if (!CheckBox_SelectStoreHouse.Checked)
                                {
                                    if (!dr[4].ToString().Trim().Equals(StoreHouseCode) ||
                                        StoreHouseCode.Trim().Equals(""))
                                    {
                                ...
                                if (RadioButtonList_Mode.SelectedValue.Equals("3"))
                                {
                                    foreach (string tbname in tablenames)
                                    {
                                        if (tbname != null && tbname != "")
                                        {
                                            ...
                                            DataTable dt = db.GetFhPiciandFhKey(tbname, dr[3].ToString().Trim(),
                                                GetCookieCompID());
                                            if (CheckBoxLabelCodeExist(tbname, dr[3].ToString().Trim()) &&
                                                dt.Rows.Count > 0)
                                            {
                                                ...
                                                sqlcmd3.CommandText = GetTuiHuoString(dr[3].ToString().Trim(), tbname,
                                                    pici, key);
                                ...
                                else
                                {
                                    foreach (string tbname in tablenames)
                                    {
                                        if (tbname != null && tbname != "")
                                        {
                                            ...
                                            if (CheckBoxLabelCodeExist(tbname, dr[3].ToString().Trim()) &&
                                                !CheckWfhISok(tbname, dr[3].ToString().Trim()))
                                            {
                                                IList<MTB_CompAgentInfo> mAgent =
                                                    bbagent.GetListsByFilterString("AgentID=" + GetCookieCompID());
                                                if (mAgent.Count > 0)
                                                {
                                                    int coid;
                                                    foreach (MTB_CompAgentInfo ma in mAgent)
                                                    {
                                                        ...
                                                        {
                                                            sqlcmd3.CommandText =
                                                                GetFaHuoSqlString(dr[3].ToString().Trim(), tbname,
                                                                    AgentName, ProductInfoID, FaHuoPiCi, StoreHouseID,
                                                ...
                                                {
                                                    sqlcmd2.CommandText = GetFaHuoSqlString(dr[3].ToString().Trim(),
                                                        tbname, AgentName, ProductInfoID, FaHuoPiCi, StoreHouseID,
                                                        fahuoinfo[3], AgentID, EnterFaHuoDate);
                        ...
                        if (oknum > 0)
                        {
                            ...
                            foreach (string line in FaHuoRecordArray)
                            {
                                if (!line.Trim().Equals(""))
                                {
                                    ...
                                    if (EnterFaHuoDate)
                                    {
                                        RecordFaHuoInfo(tablenames[0], temparray[0].Trim(), TextBox_FaHuoTime.Text,
                                    ...
                                    {
                                        RecordFaHuoInfo(tablenames[0], temparray[0].Trim(), DateTime.Now.ToString(),
                                            temparray[2].Trim(), sqlcon1, temparray[3].Trim(),
                                            Convert.ToInt32(temparray[6].Trim()), temparray[1].Trim(),
                                            temparray[4].Trim());
                                    }
    ...
    private string GetTuiHuoString(string boxstring, string tablename, string FaHuoPiCi, string FHKey)
    {
        ...
        stringtemp = "update  TB_FaHuoInfo_" + GetCookieCompID() + " set XiangNumber=XiangNumber-1  where FHPiCi='" +
                     ...
                     boxstring + "' and CompID=" + GetCookieCompID() + "and left(FHType,1)=3; delete  TB_FaHuoInfo_" +
                     GetCookieCompID() + " where XiangNumber=0 ";
        //}
        //else
        //{
        ...
        string FaHuoPiCi, string StoreHouseID, string FHKey, string AgengtID, bool InputDateTime)
    {
        ...
        if (InputDateTime)
        {
            stringtemp = " insert into " + tablename +
                         ...
                         boxstring + "'," + GetCookieCompID() + "," + StoreHouseID + "," + AgengtID + ",0,'" +
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        Session.Remove("strList");
        ScriptManager.RegisterStartupScript(updatepanel, GetType(), "info",
            "alert('清除成功！');window.location.href='FahuoNew.aspx'", true);
    }
}