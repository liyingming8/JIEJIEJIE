...
public partial class Admin_wuliu_Fahuo_FaHuoViewNew : AuthorPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //{
            //    hd_agentid.Value = Request.QueryString["pcompid"].Trim();
            //}
            //if (!string.IsNullOrEmpty(Request.QueryString["pcompnm"]))
            //{
            //    inputagentid.Value = Request.QueryString["pcompnm"].Trim();
            //}
            GetFaHuoInfo();
            //inputagentid.Attributes.Add("onclick",ReturnCompnaySelectScript("指定经销商",GetCookieCompID()));
        }
    }

    private void GetFaHuoInfo()
    {
        ...
        sqlstring += " and fh.FHDate between '" + Convert.ToDateTime(TextBox_RukuDateBegin.Text).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TextBox_RukuDateEnd.Text).AddDays(1).ToString("yyyy-MM-dd") + "'order by fh.FHDate desc ";
        //if (hd_agentid.Value != "0" && !string.IsNullOrEmpty(hd_agentid.Value))
        //{
        //    sqlstring += " and fh.AgentID=" + hd_agentid.Value;
        //}
        //if (hd_proid.Value != "0" && !string.IsNullOrEmpty(hd_proid.Value))
        //{
        //    sqlstring += " and fh.ProID=" +hd_proid.Value;
        //}
