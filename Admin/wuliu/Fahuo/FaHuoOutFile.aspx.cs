...
public partial class Admin_wuliu_Fahuo_FaHuoOutFile : AuthorPage
{
    ...
    private void FillGridViewProinfo(GridView gv)
    {
        ...
        else
        {
            ...
            if (GetCookieCompTypeID() == DAConfig.CompTypeIDChangJia.ToString().Trim() || GetCookieCompID() == "11858")
            {
                gv.DataSource = bllpro.GetListsByFilterString("CompID=" + GetCookieCompID());
            }
            else
    ...
    protected void Button4_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow GVR in GridView_TbProductsInfor.Rows)
        {
            ...
            {
                SW.WriteLine(GVR.Cells[0].Text.Trim() + "," + GVR.Cells[1].Text.Trim() + "," + AddAgentIDString(GVR.Cells[0].Text.Trim()));
            }
        }
        SW.Flush();
        SW.Close();
    ...
    private string AddAgentIDString(string productid)
    {
        _returnvalue = "";
         IList<MTJ_RegisterCompanys> complist =  comwl.ReturnGetAgentInfoByProdID(productid);
        foreach (MTJ_RegisterCompanys mtjcomp in complist)
        {
            _returnvalue += _returnvalue.Equals("") ? mtjcomp.CompID.ToString(CultureInfo.InvariantCulture) : "|" + mtjcomp.CompID;
        }
        return _returnvalue;
    }
