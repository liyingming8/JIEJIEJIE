...
public partial class Admin_TB_SY_GuanZhuangInfo : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_SY_GuanZhuangInfoAddEdit.aspx?cmd=edit&ID={0}',600,400,'灌装管理')", ID);
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_SY_GuanZhuangInfo where " + Filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_SY_GuanZhuangInfo", Filtertemp, "GuanZhuangID", "GuanZhuangID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["GuanZhuangID"].ToString()));
        // fillgridview();
        _currentindex = 1;
        AspNetPager1.CurrentPageIndex = _currentindex;
        DisplayData(1, AspNetPager1.PageSize);
