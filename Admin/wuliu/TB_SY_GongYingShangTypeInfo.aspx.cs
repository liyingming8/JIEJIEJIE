...
public partial class Admin_TB_SY_GongYingShangTypeInfo : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_SY_GongYingShangTypeInfoAddEdit.aspx?cmd=edit&ID={0}',600,400,'供应商类别管理')",
                    ID);
        }
        else
        {
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
        ...
        if (comp == "1")
        {
            ...
            AspNetPager1.RecordCount = int.Parse(_tab.ExecuteQuery("select count(*) from TB_SY_GongYingShangTypeInfo where " + Filtertemp, null).Rows[0][0].ToString());
            GridView1.DataSource = _tab.ExecuteQueryByProPagerNew(pageIndex, "TB_SY_GongYingShangTypeInfo", Filtertemp, "GYSTYPEID", "GYSTYPEID", pageSize);
            GridView1.DataBind();
        }
        else
        {
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["GYSTYPEID"].ToString()));
        // fillgridview();
        _currentindex = 1;
        AspNetPager1.CurrentPageIndex = _currentindex;
        DisplayData(1, AspNetPager1.PageSize);
