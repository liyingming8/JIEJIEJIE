...
public partial class Admin_TB_BiaoZhun : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TB_BiaoZhunAddEdit.aspx?cmd=edit&ID={0}',600,400)",
                ID);
        }
        else
        {
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["QID"].ToString()));
        fillgridview();
    }
