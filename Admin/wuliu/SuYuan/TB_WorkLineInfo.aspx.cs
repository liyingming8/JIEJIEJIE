...
public partial class Admin_wuliu_Suyan_TB_WorkLineInfo : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_WorkLineInfoAddEdit.aspx?cmd=edit&ID={0}',600,400)", ID);
        }
        else
        {
            return "";
