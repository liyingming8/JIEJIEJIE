...
public partial class Admin_TB_BaoZhuangGroup : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return
                string.Format(
                    "javascript:var win=openWinCenter('TB_BaoZhuangGroupAddEdit.aspx?cmd=edit&ID={0}',600,400)", ID);
        }
        else
        {
            return "";
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["bzid"].ToString()));
        fillgridview();
    }
