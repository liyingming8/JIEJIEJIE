...
public partial class Admin_TB_Products_InforForPrice : AuthorPage
{
    ...
    private TabExecutewuliu tabexwl = new TabExecutewuliu();
    private BTB_ProductAuthorForAgent bproauthoragent = new BTB_ProductAuthorForAgent();
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            //if (!bproauthoragent.CheckIsExistByFilterString("ProdID=" + GridView1.DataKeys[e.Row.RowIndex][0].ToString()))
            //{
            //    if (Convert.ToInt32(tabexwl.ExecuteQuery("select count(*) from TB_FaHuoInfo_" + GetCookieCompID() + " where ProID=" + GridView1.DataKeys[e.Row.RowIndex][0].ToString(), null).Rows[0][0].ToString()) == 0)
            //    {
    ...
    public string JiageLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('ProductJiageEdit.aspx?prod_id={0}',400,200)", ID);
        }
        else
        {
            return "";
