...
public partial class Admin_TB_ProductAuthorForAgent : AuthorPage
{
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["ATHPID"].ToString()));
        fillgridview();
    }
    ...
    {
        mod = bll.GetList(int.Parse(GridView1.DataKeys[e.RowIndex]["ATHPID"].ToString()));
        ...
        mod.ProdID = Convert.ToInt32(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtProdID")).Text.Trim());
        mod.CreateDate =
            Convert.ToDateTime(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtCreateDate")).Text.Trim());
        mod.ActiveDate =
            Convert.ToDateTime(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtActiveDate")).Text.Trim());
        mod.IsActive = Convert.ToBoolean(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtIsActive")).Text.Trim());
