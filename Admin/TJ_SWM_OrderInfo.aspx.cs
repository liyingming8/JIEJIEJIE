...
using TJ.BLL;
using commonlib;
public partial class Admin_TJ_SWM_OrderInfo : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('TJ_SWM_OrderInfoAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+"&ID={0}',600,450,'TJ_SWM_OrderInfo')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //if (IsSuperAdmin())
            //{
            //    ((LinkButton)e.Row.Cells[20].Controls[0]).Attributes.Add("onclick", "javascript:return confirm('你确认要删除当前记录吗?')");
            //}
            //else
            //{
            //     e.Row.Cells[20].Enabled= false;
            //     e.Row.Cells[20].ForeColor = System.Drawing.Color.LightGray;
            //}
        }
    }
