...
using TJ.BLL;
using commonlib;
public partial class Admin_tj_swm_feedback_userinfo : AuthorPage
{
    ...
    public string XiangXiLinkString(string ID)
    {
        if (ID.Length > 0)
        {
            return string.Format("javascript:var win=openWinCenter('tj_swm_feedback_userinfoAddEdit.aspx?cmd="+Sc.EncryptQueryString("edit")+ "&ID={0}',500,380,'认证中心人员')", Sc.EncryptQueryString(ID));
        }
        else
        {
            return "";
    ...
    private void DisplayData(int pageIndex, int pageSize)
    {
       ...
       AspNetPager1.RecordCount = int.Parse(tab.ExecuteQuery("select count(id) from tj_swm_feedback_userinfo where "+_filtertemp, null).Rows[0][0].ToString());
       GridView1.DataSource = tab.ExecuteQueryByProPagerNew(pageIndex, "tj_swm_feedback_userinfo", _filtertemp, "id", "id", pageSize);
       GridView1.DataBind();
    }
