...
public partial class Admin_TJ_UserCollection : AuthorPage
{
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["UCID"].ToString()));
        fillgridview();
    }
    ...
    {
        mod = bll.GetList(int.Parse(GridView1.DataKeys[e.RowIndex]["UCID"].ToString()));
        mod.UserID = Convert.ToInt32(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtUserID")).Text.Trim());
        mod.PBID = Convert.ToInt32(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtPBID")).Text.Trim());
