...
public partial class Admin_TJ_User_Terminal : AuthorPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                hd_compid.Value = Sc.DecryptQueryString(Request.QueryString["compid"]);
                DisplayData(1, AspNetPager1.PageSize);
                btn_new.Attributes.Add("onclick", "openWinCenter('TJ_UserAddEditByCompID.aspx?cmd=add&compid="+Sc.EncryptQueryString(hd_compid.Value)+"', 600, 470, '用户编辑')");
            } 
        }
    }
    ...
    public string XiangXiLinkString(string ID)
    {
        ...
        {
            //双击后编辑修改方法
            return string.Format("javascript:var win=openWinCenter('TJ_UserAddEditByCompID.aspx?compid="+Sc.EncryptQueryString(hd_compid.Value)+"&cmd=edit&ID={0}',600,470,' 用户编辑')", ID);
        }
        else
        {
            return "";
