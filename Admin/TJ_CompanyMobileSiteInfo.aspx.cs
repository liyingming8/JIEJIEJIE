...
public partial class Admin_TJ_CompanyMobileSiteInfo : AuthorPage
{
    ...
    {
        bll.Delete(int.Parse(GridView1.DataKeys[e.RowIndex]["CompSiteID"].ToString()));
        fillgridview();
    }
    ...
    {
        mod = bll.GetList(int.Parse(GridView1.DataKeys[e.RowIndex]["CompSiteID"].ToString()));
        mod.CompID = Convert.ToInt32(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtCompID")).Text.Trim());
        mod.SiteID = Convert.ToInt32(((TextBox) GridView1.Rows[e.RowIndex].FindControl("txtSiteID")).Text.Trim());
