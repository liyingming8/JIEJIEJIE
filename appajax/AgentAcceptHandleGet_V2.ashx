...
public class AgentAcceptHandlePost_V2 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (string.IsNullOrEmpty(context.Request.QueryString["masterid"]) ||
            string.IsNullOrEmpty(context.Request.QueryString["parid"]) || string.IsNullOrEmpty(context.Request.QueryString["lbcode"]) ||
            string.IsNullOrEmpty(context.Request.QueryString["agid"]) || string.IsNullOrEmpty(context.Request.QueryString["userid"]) ||
        ...
        {
            string compid = context.Request.QueryString["masterid"];
            string parentid = context.Request.QueryString["parid"];
            string labelcode = context.Request.QueryString["lbcode"];
            string agent_id = context.Request.QueryString["agid"];
            string user_id = context.Request.QueryString["userid"];
            string pid = context.Request.QueryString["pid"];
            string smaddress = "";
            if(!string.IsNullOrEmpty(context.Request.QueryString["smplace"]))
            {
                smaddress = context.Request.QueryString["smplace"];
                db.Penglog(context.Request.QueryString["smplace"], "appupload");
            }
            try
            {
                string sqlstring = "select count(id) from " + _totablename + " where BoxLabel=" + labelcode+" and isexception=0";
