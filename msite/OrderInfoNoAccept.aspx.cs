...
public partial class msite_OrderInfoNoAccept : MAuthorPage
{
    ...
    private void GetOrderInfo()
    {
        ...
        if (authorgoodsidstring.Length > 0)
        {
            orderlist =
                borderinfo.GetListsByFilterString(
                    "OrderStatusID=0 and (DeliveryCompID is null or DeliveryCompID=0) and (CompID=" + GetCookieCompID() +
                    " or GoodsID in(" + authorgoodsidstring + "))");
        }
        else
        {
            orderlist =
        ...
        foreach (MTJ_OrderInfo morder in orderlist)
        {
            ...
            sb.Append("             <div class=\"col-xs-3 prodpic\"><img src=\"../Admin/" + mgood.GoodsPicURLSmal +
                      "\" /></div>");
            sb.Append("             <div class=\"col-xs-5 goodsname\"><div>产品:" + mgood.GoodsName + "</div><div>数量：" +
                      morder.OrderNum + ";" + mgood.SaleUnitID + "</div><div>售价：￥" + morder.ShoulPayMoney +
                      "</div></div>");
            sb.Append("             <div class=\"col-xs-4\"><a href=\"JieDan.aspx?orderid=" + morder.OrderID +
                      "\"  class=\"ButtonOK\">我来接单</a></div>");
