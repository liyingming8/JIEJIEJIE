<%@ WebHandler Language="C#" Class="getactivityinfo_v3" %>
using System;
using System.Data;
...
public class getactivityinfo_v3 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
            ...
            !string.IsNullOrEmpty(context.Request.QueryString["uid"]))
        {
            ...
            //判断是否扫过
            if (!checkcodeisqueried(code))
            {
                string[] array = GetBoxLabelCode("AgentAcceptLabelCodeInfo_2019", code);
                if (null != array)
                {
                    DataTable dttemp = GetShouHuoConfirmInfo(array[0], "AgentAcceptInfo_2019", compid);
                    if (dttemp.Rows.Count > 0)
                    {
                        string acidstring = CheckActivityInfo(compid, dttemp.Rows[0]["pid"].ToString(),
                            dttemp.Rows[0]["agentid"].ToString(), dttemp.Rows[0]["terminalid"].ToString());
                        if (acidstring != "0")
                        {
                            string acidDate = GetPermitPrizeActivityID(acidstring, array[2], Convert.ToDateTime((string.IsNullOrEmpty(array[1]) ? DateTime.Now.AddMonths(-1).ToString() : array[1])).ToString("yyyy-MM-dd"));
                            string[] acidcreatearray = acidDate.Split(new[] { ',' },
                                StringSplitOptions.RemoveEmptyEntries);
                            if (acidcreatearray.Length.Equals(2))
                            {
                                string temp = GetAcStrategyString(acidcreatearray[0], code, Convert.ToDateTime(acidcreatearray[1]).Year.ToString(), compid, DateTime.Now, userid, array[0], "");
    ...
    public string CheckActivityInfo(string compid, string prodid, string agentid, string terminalid)
    {
        using (var conn = GetConnection)
        {
            _tempsqlstring = "select ac.id from TJ_Activity ac where ac.CompID=" + compid + " and ac.FaceTo='1' and ('" + DateTime.Now.ToString("yyyy-MM-dd") + "' between ac.STM and ac.ETM) and  ((ac.prodnum=0 or (select count(id) from TJ_Activity_Product p where p.prodid=" + prodid + " and p.acid=ac.id)>0))  and  ((ac.agentnum=0 or (select count(ag.id) from TJ_Activity_Agent ag where ag.agentid=" + agentid + " and ag.acid=ac.id)>0)) and  ((ac.terminalnum=0 or (select count(at.id) from TJ_Activity_Terminal at where at.terminalid=" + terminalid + " and at.acid=ac.id)>0))";
            dtable = new DataTable();
            MyAda = new SqlDataAdapter(_tempsqlstring, conn);
            MyAda.Fill(dtable);
            if (dtable.Rows.Count > 0)
    ...
    private DataTable GetShouHuoConfirmInfo(string boxlabelcode, string tablename, string compid)
    {
        using (var conn = GetConnectionWL)
        {
            _tempsqlstring = "select ProID as pid,ParentID as agentid,AcceptAgentID terminalid, AcceptDate tm,KhDDH from " + tablename + " where CompID=" + compid + " and BoxLabel='" + boxlabelcode + "'";
            dtable = new DataTable();
            MyAda = new SqlDataAdapter(_tempsqlstring, conn);
            MyAda.Fill(dtable);
            MyAda.Dispose();
    ...
    private bool checkcodeisqueried(string cd)
    {
        using (var conn = GetConnection)
        {
            _tempsqlstring = "select ID from TJ_SMinfo_2018 where LabelCode='" + cd + "'";
            MyAda = new SqlDataAdapter(_tempsqlstring, conn);
            dtable = new DataTable();
            MyAda.Fill(dtable);
            if (dtable.Rows.Count > 0)
