<%@ WebHandler Language="C#" Class="getactivityinfo_v2" %>
using System;
using System.Data;
...
public class getactivityinfo_v2 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
            ...
            !string.IsNullOrEmpty(context.Request.QueryString["uid"]))
        {
            ...
            if (type == "acid")
            {
                string boxlabelandtable = GetTableAndCpidNew(code);
                if (boxlabelandtable.Contains(","))
                {
                    ...
                    if (boxtablearray.Length > 0)
                    {
                        ...
                        if (boxlabelcode != code)
                        {
                            DataTable dttemp = Getfhinfo(boxlabelcode, tablename, compid);
                            if (dttemp.Rows.Count > 0)
                            {
                                string acidstring = CheckActivityInfo(compid, dttemp.Rows[0]["ProID"].ToString(),
                                    dttemp.Rows[0]["AgentID"].ToString());
                                if (acidstring != "0")
                                {
                                    string acidDate = GetPermitPrizeActivityID(acidstring,
                                        dttemp.Rows[0]["KhDDH"].ToString(),
                                    ...
                                    {
                                        result = "{\"flag\":\"1\",\"acid\":\"" + acidcreatearray[0] + "\",\"acyear\":\"" +
                                                 Convert.ToDateTime(acidcreatearray[1]).Year + "\",\"proid\":\"" +
                                                 dttemp.Rows[0]["ProID"] + "\",\"agentid\":\"" +
                                                 dttemp.Rows[0]["AgentID"] + "\"," +
                                                 "\"khddh\":\"" + dttemp.Rows[0]["KhDDH"] + "\",\"boxcode\":\"" +
                                                 boxlabelcode + "\",\"table\":\"" + tablename + "\"}";
            ...
            else
            {
                ...
                string temp = GetAcStrategyString(acid, code, acyear, compid, DateTime.Now, userid, boxcode, table);
    ...
    private DataTable Getfhinfo(string boxlabelcode, string tablename, string compid)
    {
        using (var conn = GetConnectionWL)
        {
            _tempsqlstring = "select top 1 FHKey from " + tablename + "_FH where BoxLabel01='" + boxlabelcode + "'  and FromAgentID=" + compid + " order by ID";
            dtable = new DataTable();
            MyAda = new SqlDataAdapter(_tempsqlstring, conn);
            MyAda.Fill(dtable);
            MyAda.Dispose();
